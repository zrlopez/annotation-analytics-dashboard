...
import { buildDateSeries, buildFutureDates } from './utils'

const dailyDates = buildDateSeries(25)
const historyDates = buildDateSeries(10)
const futureDates = buildFutureDates(10)
const alertDates = buildDateSeries(5)
...
export const data: AppData = {
  ...
  teamData: {
    members: [
      { name: 'Analyst A', avatar: 'A', productivity: 94, tasksCompleted: 127, avgTaskTime: 2.3 },
      { name: 'Analyst B', avatar: 'B', productivity: 91, tasksCompleted: 118, avgTaskTime: 2.6 },
      { name: 'Analyst C', avatar: 'C', productivity: 89, tasksCompleted: 115, avgTaskTime: 2.8 },
      { name: 'Analyst D', avatar: 'D', productivity: 87, tasksCompleted: 109, avgTaskTime: 3.1 },
      { name: 'Analyst E', avatar: 'E', productivity: 85, tasksCompleted: 102, avgTaskTime: 3.4 },
      { name: 'Analyst F', avatar: 'F', productivity: 83, tasksCompleted: 98, avgTaskTime: 3.6 }
    ],
    productivity: dailyDates.map((date, i) => ({
      date,
  ...
  capacityData: {
    forecast: futureDates.map((date, i) => ({
      date,
  ...
  alertsData: {
    active: [
      { id: 1, message: 'High error rate detected in Data Processing', priority: 'high', timestamp: new Date(Date.now() - 60 * 60 * 1000).toISOString(), status: 'active' },
      { id: 2, message: 'Capacity utilization exceeded 90% threshold', priority: 'medium', timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(), status: 'active' },
      { id: 3, message: 'Team productivity below target for 2 hours', priority: 'medium', timestamp: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(), status: 'acknowledged' },
      { id: 4, message: 'Database connection pool near limit', priority: 'high', timestamp: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(), status: 'active' }
    ],
    history: alertDates.map((date, i) => ({
      date,
